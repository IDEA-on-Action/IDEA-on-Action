...
- **Fixed Supabase Linter Warnings**:
    ...
    - Secured `minu_integration_view` and `compass_integration_view` by restricting access to `service_role` and enabling `security_invoker`.
    - Secured `rag_index_stats` and `project_type_stats` by enabling `security_invoker`.
- **Hardening**:
