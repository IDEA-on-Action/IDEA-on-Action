...
-- Final version: UUID 타입 대응, 새 데이터 INSERT 방식
-- Required URLs:
...
-- Create index on slug for faster lookups (먼저 생성)
CREATE INDEX IF NOT EXISTS idx_services_slug ON public.services(slug);

-- Insert MVP Development Service
...
자세한 환불 규정은 [환불정책 페이지](https://www.ideaonaction.ai/refund-policy)를 참고해주세요.
...
-- Insert Fullstack Development Service
INSERT INTO public.services (
  slug,
  title,
  description,
...
-- Insert Operations Management Service
INSERT INTO public.services (
  slug,
  title,
  description,
...
DECLARE
  service_count INT;
BEGIN
  SELECT COUNT(*) INTO service_count FROM public.services WHERE slug IN ('mvp', 'fullstack', 'design', 'operations');

  RAISE NOTICE '=== Toss Payments Review Services Created ===';
  RAISE NOTICE 'Total services with required slugs: %', service_count;
  RAISE NOTICE '';
  RAISE NOTICE 'URLs:';
  ...
  RAISE NOTICE '';
  RAISE NOTICE 'Pricing: https://www.ideaonaction.ai/pricing';
  RAISE NOTICE 'Refund Policy: https://www.ideaonaction.ai/refund-policy';
END $$;
