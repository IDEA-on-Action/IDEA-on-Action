...
결제 처리를 위한 Edge Function 추가.

#### 생성/수정된 파일
- `supabase/functions/_shared/response.ts`: 공유 응답 유틸리티
- `supabase/functions/payment-webhook/`: 웹훅 처리
- `supabase/functions/toss-payment/`: 결제 승인 처리

### 🔧 기타 개선

- `date-range-picker.tsx`: "오늘" 프리셋 버튼 추가
- `services.ts`: 서비스 타입 정의 확장
- MCP 토큰 스키마 마이그레이션 추가
- CI 워크플로우 PR 코멘트 권한 수정

### 🧪 테스트 추가

- E2E: `services-list.spec.ts`, `mvp-service.spec.ts`
- Unit: `xlsx-chart-insert.test.ts`, `docx-image-insert.test.ts`
...
- Index.tsx 홈 페이지 재구성
- E2E 테스트 신규 작성 (navigation.spec.ts, redirects.spec.ts)
...
### 📦 Stats
- 신규 파일: 14개 (허브 3, 테스트 2, 문서 9)
- 수정 파일: 9개 (App.tsx, Header.tsx, Index.tsx 등)
- 빌드: 17.45s (PWA precache 27 entries)
