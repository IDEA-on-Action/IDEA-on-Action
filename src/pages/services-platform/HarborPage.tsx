...
import { compassHarborService } from "@/data/services/compass-harbor";
import {
  CheckCircle2,
  AlertCircle,
  Anchor,
...
function useMCPSubscriptionFallback(
  ...
  // COMPASS Harbor 서비스에 대한 구독만 필터링
  const harborSubscription = useMemo(() => {
    if (!subscriptions) return null;

    // compass-harbor 서비스 ID 또는 slug로 필터링
    const found = subscriptions.find(
      (sub) =>
        sub.service?.slug === "harbor" ||
        sub.service?.id === "compass-harbor" ||
        sub.service?.title?.includes("Harbor")
    );
...
export default function HarborPage() {
  const service = compassHarborService;
  ...
  //   endpoint: 'compass/harbor/subscription',
  //   userId: user?.id,
  //   enabled: !!user,
  // });
  // =====================================================
  ...
  return (
    <PageLayout>
      <SEO
        ...
        canonical="/services/compass/harbor"
        ogType="service"
        service={{
          name: service.title,
          description: service.description,
        ...
        breadcrumbs={[
          ...
          { name: service.title, url: "/services/compass/harbor" },
        ]}
      />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
          <div className="glass-card p-6 rounded-lg">
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="text-lg font-semibold">실시간 모니터링</span>
            </div>
            <p className="text-muted-foreground">
              시스템 상태, 성능 지표, 리소스 사용량을 실시간 대시보드로 모니터링합니다.
            </p>
          </div>
          <div className="glass-card p-6 rounded-lg">
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="text-lg font-semibold">장애 알림</span>
            </div>
            <p className="text-muted-foreground">
              이메일, Slack, SMS, PagerDuty 등 다양한 채널로 즉시 알림을 받습니다.
            </p>
          </div>
          <div className="glass-card p-6 rounded-lg">
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="text-lg font-semibold">티켓 관리</span>
            </div>
            <p className="text-muted-foreground">
              고객 지원 요청을 체계적으로 관리하고 SLA 기준에 맞게 처리합니다.
            </p>
          </div>
          <div className="glass-card p-6 rounded-lg">
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="text-lg font-semibold">SLA 추적</span>
            </div>
            <p className="text-muted-foreground">
              고객별 SLA 이행률을 자동 추적하고 월간 보고서를 생성합니다.
            </p>
          </div>
          <div className="glass-card p-6 rounded-lg">
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="text-lg font-semibold">자동화 유지보수</span>
            </div>
            <p className="text-muted-foreground">
              정기 점검, 백업, 보안 패치 등 유지보수 작업을 자동화합니다.
            </p>
          </div>
          <div className="glass-card p-6 rounded-lg">
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="text-lg font-semibold">비용 분석</span>
            </div>
            <p className="text-muted-foreground">
              인프라 비용을 분석하고 최적화 방안을 제안합니다.
            </p>
          </div>
        </div>
      </Section>
          ...
          <div className="max-w-4xl mx-auto">
            <div className="space-y-6">
              {service.process.map((step) => (
                ...
                >
                  <div className="flex-shrink-0 w-10 h-10 rounded-full bg-gradient-to-br from-green-500 to-emerald-500 text-white flex items-center justify-center font-bold">
                    {step.step}
                  </div>
      ...
      <Section title="플랜 비교">
        ...
        {user && !isLoading && mcpSubscription && (
          <div className="mb-6 p-4 bg-green-500/5 border border-green-500/20 rounded-lg max-w-2xl mx-auto">
            <div className="flex items-center gap-3">
              <CheckCircle2 className="h-5 w-5 text-green-500 flex-shrink-0" />
              <div>
                <p className="font-medium">
                  현재 <span className="text-green-500">{mcpSubscription.planName}</span> 플랜을 이용 중입니다
                </p>
      ...
      {/* SLA Table */}
      <Section title="SLA 기준">
        <div className="max-w-4xl mx-auto overflow-x-auto">
          <table className="w-full border-collapse border border-border">
            <thead className="bg-muted">
              <tr>
                <th className="border border-border px-4 py-2 text-left">플랜</th>
                <th className="border border-border px-4 py-2 text-left">모니터링 서비스</th>
                <th className="border border-border px-4 py-2 text-left">알림 채널</th>
                <th className="border border-border px-4 py-2 text-left">데이터 보관</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td className="border border-border px-4 py-2 font-semibold">Basic</td>
                <td className="border border-border px-4 py-2">5개</td>
                <td className="border border-border px-4 py-2">이메일</td>
                <td className="border border-border px-4 py-2">7일</td>
              </tr>
              <tr>
                <td className="border border-border px-4 py-2 font-semibold">Pro</td>
                <td className="border border-border px-4 py-2">20개</td>
                <td className="border border-border px-4 py-2">이메일, Slack, SMS</td>
                <td className="border border-border px-4 py-2">90일</td>
              </tr>
              <tr>
                <td className="border border-border px-4 py-2 font-semibold">Enterprise</td>
                <td className="border border-border px-4 py-2">무제한</td>
                <td className="border border-border px-4 py-2">전체 + PagerDuty</td>
                <td className="border border-border px-4 py-2">365일</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <Section title="COMPASS 통합 연동">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
          <div className="glass-card p-6 rounded-lg text-center">
            ...
            <h3 className="font-semibold mb-2">Captain 연동</h3>
            <p className="text-sm text-muted-foreground">
              완료된 프로젝트를 자동으로 Harbor로 이관하여 운영을 시작합니다.
            </p>
          </div>
          <div className="glass-card p-6 rounded-lg text-center">
            ...
            <p className="text-sm text-muted-foreground">
              기존 고객 운영 중 추가 프로젝트 기회를 Navigator로 관리합니다.
            </p>
          </div>
          <div className="glass-card p-6 rounded-lg text-center">
            ...
            <h3 className="font-semibold mb-2">통합 리포팅</h3>
            <p className="text-sm text-muted-foreground">
              전체 COMPASS 서비스의 데이터를 통합 분석하여 비즈니스 인사이트를 제공합니다.
            </p>
          </div>
        </div>
      </Section>
        ...
        <Section title="출시 알림 신청">
          <div className="max-w-2xl mx-auto glass-card p-6 rounded-lg">
            <p className="mb-4 text-center">
              COMPASS Harbor는 현재 개발 중입니다. 출시 알림을 신청하시면 가장
              먼저 소식을 받아보실 수 있습니다.
            </p>
      ...
      <CTASection
        primary={
          isComingSoon
            ? { label: "출시 알림 신청하기", href: "/work-with-us?service=harbor" }
            : { label: "무료 체험 시작하기", href: "/signup?plan=harbor-trial" }
        }
        secondary={{
 ...
 * - Coming Soon: 모든 플랜에 "출시 알림 신청" 버튼
 */
    ...
    if (isComingSoon) {
      return (
        <Button variant="secondary" className={baseClasses} asChild>
          <a href={`/work-with-us?service=harbor&plan=${plan.name.toLowerCase()}`}>
            출시 알림 신청
          </a>
        </Button>
      );
    ...
    switch (status) {
      ...
      default:
        ...
        return (
          ...
          >
            <a href={isLoggedIn
              ? `/subscriptions/checkout?service=compass-harbor&plan=${plan.name.toLowerCase()}`
              : `/signup?redirect=/subscriptions/checkout?service=compass-harbor&plan=${plan.name.toLowerCase()}`
            }>
              시작하기
            </a>
          </Button>
        ...
        return (
          <div
            ...
            className={cn(
              ...
              plan.recommended && !isCurrentPlan && "ring-2 ring-green-500/50 ring-offset-2"
            )}
          >
            {/* 플랜 헤더 */}
            ...
            <ul className="space-y-2 pt-4 border-t text-sm">
              ...
              <li className="flex items-center gap-2">
                <CheckCircle2 className="h-4 w-4 text-green-500 flex-shrink-0" />
                <span>알림 채널: {plan.features.alertChannels}</span>
              </li>
              <li className="flex items-center gap-2">
                <CheckCircle2 className="h-4 w-4 text-green-500 flex-shrink-0" />
                <span>데이터 보관: {plan.features.retentionPeriod}</span>
              </li>
              <li className="flex items-center gap-2">
                <CheckCircle2 className="h-4 w-4 text-green-500 flex-shrink-0" />
